...
Critical e2e (ci.yml e2e-critical): event-studio-map, supply-nav, search-filters, search-persist, flow, deal-path, cabinets-cross-role, cabinet-customer, cabinet-performer, cabinet-venue, org-switch, cabinet-a11y, studio-autosave, onboarding.

## Evidence table

| ID | Scenario | Command / test path | Result | SHA / evidence | Notes |
| ---- | ---- | ---- | ---- | ---- | ---- |
| E01 | Гость ищет артиста → входит | search-filters + e2e/search-persist.spec.ts | PASS | local+CI list | Real UI login + `next`; 429 retry |
...
| E07 | Заявка → оффер одна сущность | e2e/deal-path.spec.ts E07; flow; cross-role | PASS | local | In CI critical list |
| E08 | Новая версия оффера | test_quote_versioning.py; deal-path E08 | PASS | local | Old acks invalid until re-ack |
| E09 | Двойной hold | test_hold_race.py; deal-path E09 | PASS | local | One 200 + one 409 |
...
| E12 | Fake pay redirect / webhook | test_payments.py stub | PARTIAL | base | stub PASS; live PSP OWNER_BLOCKED |
| E13 | External-paid + audit | test_external_payment_confirm.py | PASS | e578301 | |
| E14 | Refund idempotency | authz refunds | PARTIAL | base | stub/full refund; live adapter OWNER_BLOCKED |
| E15 | Три роли | cabinets-cross-role; e2e/org-switch.spec.ts | PASS | local | Org pin across hard nav |
...
| E24 | 390 + keyboard | cabinet-a11y.spec.ts; docs/screenshots/g2-roles/ | PASS | local | Full SR audit still manual |
...
| ---- | ---- |
| CI green on candidate SHA | Expanded e2e-critical (+cab depth); await green after push |
...
| Wave2 cab depth | cabinet-customer/performer/venue.spec.ts |
