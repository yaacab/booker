...
Web gateway routes return 404 unless `BOOKER_DEMO_GATEWAY=1` is set on the web service. `/dev/cabinets` opens customer, performer, venue and operator roles using existing seeded test accounts. Tokens are kept in sessionStorage, isolated from real tokens and organization selection. Demo drafts use sessionStorage as well. Test data is shared and must not contain personal information.

To disable access, unset BOOKER_DEMO_GATEWAY and restart booker-web, then stop booker-demo-api. No rebuild is needed. Existing production authentication is unchanged.
