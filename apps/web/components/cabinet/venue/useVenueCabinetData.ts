...
export function useVenueCabinetData() {
  ...
  const [dealRooms, setDealRooms] = useState<VenueDealRoom[]>([]);
  const [completeness, setCompleteness] = useState<ProfileCompleteness | null>(null);
  const [halls, setHalls] = useState<VenueHallTarget[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [rq, bk, comp, targets] = await Promise.all([
        api<{ items: VenueRequest[] }>(`/requests${q}`),
        api<{ items: VenueBooking[] }>(`/bookings${q}`),
        api<ProfileCompleteness>(`/organizations/${encodeURIComponent(org.id)}/supply-completeness`),
        api<{ items: VenueHallTarget[] }>(`/organizations/${encodeURIComponent(org.id)}/calendar-targets`),
      ]);
      setRequests(rq.items);
      setBookings(bk.items);
      setCompleteness(comp);
      setHalls(targets.items);
      const activeBookings = bk.items.filter((b) => ACTIVE_DEAL_STATUSES.has(b.status));
      const rooms = await Promise.all(
        activeBookings.map((b) =>
          api<VenueDealRoom>(`/deal-room/${b.id}`).catch(() => null),
        ),
      );
      setDealRooms(rooms.filter((r): r is VenueDealRoom => r != null));
      trackClientEvent("cabinet.viewed", { kind: "venue" });
      setError("");
    } catch (err) {
  ...
  async function sendOffer(item: VenueRequest) {
    if (!item.slot_id) {
