    ...
    <main className="public-profile-reference venue-profile-reference page-enter">
      ...
      <div className="public-profile-layout">
        <aside className="public-profile-rail">
          <ProfileMedia src={data.photos?.[0]?.url || data.media_url} name={data.name} />
          {data.photos && data.photos.length > 1 ? <div className="venue-photo-gallery">{data.photos.slice(1).map(photo => <img key={photo.url} src={photo.url} alt={data.name} loading="lazy" referrerPolicy="no-referrer" />)}</div> : null}
        ...
        <div className="public-profile-content">
          ...
          <section className="public-profile-section">
            <h2>О площадке</h2>
            {data.public_disclosure ? <p className="profile-calendar-notice">{data.public_disclosure}</p> : null}
            {data.official_website ? <p><a href={data.official_website} target="_blank" rel="noopener noreferrer">Официальный сайт площадки ↗</a></p> : null}
