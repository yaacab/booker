...
export function WorkspaceNavigation({ mode, admin }: { mode: CabinetMode | null; admin: boolean }) {
  ...
  const links = !role ? [["/cabinet", "Моё пространство"]] : role === "customer"
    ...
    : [[root,"Обзор"],...(role==="performer"?[["/briefs","Подобрать выступления"]]:[]),[`${root}/calendar`,"Календарь"],[`${root}/requests`,"Заявки"],[`${root}/messages`,"Сообщения"],...(role === "venue" ? [[`${root}/halls`,"Залы"],[`${root}/stats`,"Статистика"]] : [[`${root}/services`,"Услуги"]])];
